...
import { Download, Share, ArrowLeft, Printer, ShoppingBag } from "lucide-react";
...
const UserBillPage = () => {
  ...
  const [generateTableBillMutation, { isLoading: isGeneratingBill }] =
    useGenerateTableBillMutation();
  ...
  const handleDownloadBill = async () => {
    ...
    try {
      await generateTableBillMutation({ tableId }).unwrap();
    } catch (error) {
      errorToast({ error, message: "Failed to generate bill" });
    }
    ...
    if (navigator.share) {
      navigator
        .share({
          ...
          text: `My bill from ${restaurantDetails?.restroName || "Restaurant"} for table ${tableDetails?.title || "N/A"} is ${numberToCurrency(total, "INR", 2)}`,
        })
        .catch(() => {});
    }
  };
      ...
      <main className="container max-w-md mx-auto px-4 py-6">
        ...
        <div className="flex justify-between gap-2 print:hidden">
          ...
          <Button
            ...
            disabled={isGeneratingPdf || isGeneratingBill}
          >
            <Download className="mr-2 h-4 w-4" />
            {isGeneratingPdf || isGeneratingBill ? "Generating..." : "Download"}
          </Button>
          <Button
            variant="outline"
            className="flex-1"
